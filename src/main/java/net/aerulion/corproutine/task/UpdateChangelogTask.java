...
public class UpdateChangelogTask extends BukkitRunnable {
    ...
    public void run() {
        try (Connection connection = MySQLUtils.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO `stafftool`.`tool_changelog` (toolname,id_intool,type,changeby,meta) VALUES (?,?,?,(SELECT uid FROM tool_users WHERE username=?),?)");
            ...
            PreparedStatement preparedStatement2 = connection.prepareStatement("UPDATE `stafftool`.`tool_datacache` SET cache = (SELECT CONCAT('[', GROUP_CONCAT(JSON_OBJECT('id', id, 'task', task, 'category', category, 'nextdate', nextdate) separator ','), ']') FROM `stafftool`.`tool_routine` WHERE nextdate <= UNIX_TIMESTAMP()) WHERE title = 'routine_urgent'");
