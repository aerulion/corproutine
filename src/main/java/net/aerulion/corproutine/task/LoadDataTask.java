        ...
        try (Connection connection = MySQLUtils.getConnection()) {
            ...
            while (resultSet.next()) {
                Main.ROUTINE_TASKS.put(resultSet.getInt(1),
                        new RoutineTask(resultSet.getInt(1),
                                resultSet.getString(4),
                                resultSet.getString(3),
                                Util.convertDate(resultSet.getString(5)),
                                resultSet.getString(7),
                                resultSet.getString(2),
                                Util.convertNames(resultSet.getString(6))));
            }
            if (player != null)
                Bukkit.getScheduler().runTask(Main.plugin, () -> player.openInventory(Inventories.MainMenu()));
