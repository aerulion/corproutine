...
public class UpdateDataTask extends BukkitRunnable {
    ...
    public UpdateDataTask(EditSession editSession) {
    ...
    public void run() {
        try (Connection connection = MySQLUtils.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement("UPDATE `stafftool`.`tool_routine` SET nextdate=?,done_by=?,comment=? WHERE id=?");
            ...
            new UpdateChangelogTask(EDIT_SESSION.getRoutineID(), Bukkit.getOfflinePlayer(EDIT_SESSION.getSessionOwner()).getName(), Main.ROUTINE_TASKS.get(EDIT_SESSION.getRoutineID()).getName());
        } catch (SQLException exception) {
            ConsoleUtils.sendColoredConsoleMessage(Messages.ERROR_DATABASE.get());
            Player player = Bukkit.getPlayer(EDIT_SESSION.getSessionOwner());
            if (player != null) {
